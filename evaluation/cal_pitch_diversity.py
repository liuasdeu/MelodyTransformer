...
if __name__ == '__main__':
    ...
    pitches_diversity_hyp = []
    pitches_diversity_gt = []
    durations_diversity_hyp = []
    durations_diversity_gt = []
    ...
    for filename in os.listdir(f'{hyp_prefix}/'):
        ...
        unique_duration_hyp = set(hyp_dur)
        unique_duration_gt = set(gt_dur)

        unique_pitch_hyp = set(hyp_pitch)
        # print(unique_pitch_hyp)
        unique_pitch_gt = set(gt_pitch)

        pitches_diversity_hyp.append(float(len(unique_pitch_hyp)) / float(len(hyp_pitch)))
        pitches_diversity_gt.append(float(len(unique_pitch_gt)) / float(len(gt_pitch)))

        durations_diversity_hyp.append(float(len(unique_duration_hyp)) / float(len(hyp_dur)))
        durations_diversity_gt.append(float(len(unique_duration_gt)) / float(len(gt_dur)))

    pitches_diversity_hyp = np.array(pitches_diversity_hyp)
    pitches_diversity_gt = np.array(pitches_diversity_gt)
    durations_diversity_hyp = np.array(durations_diversity_hyp)
    durations_diversity_gt = np.array(durations_diversity_gt)
    print('PITCH_hyp', np.mean(pitches_diversity_hyp), np.std(pitches_diversity_hyp))
    print('PITCH_gt', np.mean(pitches_diversity_gt), np.std(pitches_diversity_gt))
    print('DUR_hyp', np.mean(durations_diversity_hyp), np.std(durations_diversity_hyp))
    print('DUR_gt', np.mean(durations_diversity_gt), np.std(durations_diversity_gt))
