...
def select_melody_nochord(is_maj, is_chorus, length, last_bar, chord, chord_ptr,is_last_sentence):
    cursor = c.execute(
        "SELECT DISTINCT NOTES,CHORDS from MELOLIB  where LENGTH = '{}' and CHORUS = '{}' and MAJOR = '{}' ".format(
            length, is_chorus, is_maj))  # and MAJOR = '{}'
    candidates_bars = []
                    ...
                    if cnt < 9 and len(selected_bars) > 0:
                        ...
                        while cnt > 0:
                            ...
                            selected_bars = select_melody(is_maj, is_chorus, split_len, last_bar, chord, chord_ptr,
                                                          is_last_sentence)
                            if len(selected_bars) == 0:
                                selected_bars = select_melody_nochord(is_maj, is_chorus, split_len, last_bar,chord, chord_ptr,
                                                                      is_last_sentence)
                                if len(selected_bars) > 0:
